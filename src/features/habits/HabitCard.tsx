...
export function HabitCard({ habit, isCompleted, currentValue, onToggle, onValueChange, onEdit }: HabitCardProps) {
    ...
    return (
        <motion.div
            layout
            className={cn(
                "group relative flex flex-col items-stretch gap-1.5 lg:gap-3 p-3.5 lg:p-6 rounded-[16px] lg:rounded-[20px] transition-all duration-300 mb-2 overflow-hidden",
        ...
        >
            <div className="flex items-start justify-between w-full">
                <div className="flex flex-row items-center gap-2.5 lg:gap-[10px] min-w-0 flex-1">
                    ...
                    <div className="flex flex-col justify-center min-w-0 flex-1">
                        ...
                        <div className="mt-0.5 lg:mt-1.5 pl-8 lg:pl-[2.75rem] min-w-0">
                            {habit.type === 'boolean' ? (
                                <span className={cn(
                                ...
                                <div className="flex flex-col gap-1 w-full mt-0.5 lg:mt-1">
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5 lg:mt-1">
                                        <div className="w-24 lg:w-32 h-1 lg:h-1.5 rounded-full bg-white/10 overflow-hidden shrink-0">
                                            <div
                                    ...
                                    <div className="flex gap-1.5 mt-1 lg:mt-2 overflow-x-auto custom-scrollbar pb-1">
                                        <button
                                            onClick={(e) => { e.stopPropagation(); onValueChange(Math.max(0, (currentValue || 0) - 5)); }}
                                            className="px-2.5 py-1 text-[10px] lg:text-xs rounded-lg bg-white/5 border border-white/10 text-white/50 hover:bg-white/10 hover:text-white/70 transition-all shrink-0"
                                        >
                                            -5
                                        </button>
                                        <button
                                            onClick={(e) => { e.stopPropagation(); onValueChange((currentValue || 0) + 10); }}
                                            className="px-2.5 py-1 text-[10px] lg:text-xs rounded-lg bg-cyan-400/15 border border-cyan-400/25 text-cyan-400/80 hover:bg-cyan-400/25 hover:text-cyan-400 transition-all font-medium shrink-0"
                                        >
                                            +10
                                        </button>
                                        {(currentValue || 0) >= habit.goal && (
                                            <button
                                                ...
                                                className="px-2.5 py-1 text-[10px] lg:text-xs rounded-lg bg-cyan-400/20 border border-cyan-400/40 text-cyan-400 hover:bg-cyan-400/30 transition-all font-medium ml-auto shrink-0"
                                            >
                                                ✓ Listo
                                            </button>
                                        )}
                ...
                <button
                    ...
                    className="opacity-0 group-hover:opacity-100 p-1 lg:p-2.5 text-neutral-500 hover:text-cyan-400 transition-all rounded-xl hover:bg-white/10 bg-white/5 lg:bg-transparent cursor-pointer shrink-0 ml-1 lg:ml-2"
                    title="Editar Hábito"
                >
