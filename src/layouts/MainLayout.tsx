...
import { LayoutDashboard, Wallet, PieChart, LogIn, LogOut, Plus, User, Brain, ListTodo, ShoppingBag } from 'lucide-react';
...
const featureColors: Record<string, { icon: string; activeBg: string; mobileActiveBg: string; dot: string }> = {
    ...
    '/store':    { icon: 'text-rose-400',    activeBg: 'bg-rose-500/[0.10]',    mobileActiveBg: 'bg-rose-500/15',    dot: 'bg-rose-400'    },
};
    ...
    return (
        <div className="flex min-h-screen bg-[#050505] text-white font-sans selection:bg-cyan-500/30">
            ...
            <aside className="hidden lg:flex flex-col items-center w-[68px] fixed left-0 top-0 bottom-0 z-50 bg-[#0a0a0a]/95 backdrop-blur-xl border-r border-white/[0.05]">
                ...
                {/* Nav items */}
                <nav className="flex-1 flex flex-col items-center gap-1 px-3 py-1 overflow-y-auto">
                        ...
                        {mobileNavItems.map((item) => {
                            ...
                            return (
                                <button
                                    ...
                                    onClick={() => navigate(item.path)}
                                    className={cn(
                                        "relative flex flex-col items-center justify-center flex-1 h-14 min-w-0 rounded-[1.5rem] transition-all duration-300",
